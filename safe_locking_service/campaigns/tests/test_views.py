...
class TestCampaignViews(TestCase):
    ...
    def test_campaign_view(self):
        ...
        self.assertEqual(
            campaign_response.get("campaignId"), campaign_expected.campaign_search_id
        )
    ...
    def test_sort_campaign_view(self):
        ...
        campaign_response = response_json["results"][0]
        self.assertEqual(
            campaign_response.get("campaignId"), last_campaign.campaign_search_id
        )
    ...
    def test_retrieve_campaign_view(self):
        ...
        response = self.client.get(
            reverse(
                "v1:locking_campaigns:retrieve-campaign", args=(campaing_search_id,)
            ),
            format="json",
        )
        ...
        campaign_search_id = campaign_expected.campaign_search_id

        response = self.client.get(
            reverse(
                "v1:locking_campaigns:retrieve-campaign", args=(campaign_search_id,)
            ),
            format="json",
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
